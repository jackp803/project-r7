"""Deterministic closed historical Candle sequence for the OKX E1 slice."""
...
from .okx import normalize_okx_history_page
from .timeframes import is_timeframe_aligned, timeframe_duration


class HistoricalPageSource(Protocol):
    def fetch_page(
        self,
        *,
        symbol: str,
...
def validate_historical_sequence(
    ...
    *,
    ...
    This validator never sorts or repairs input. Duplicate, out-of-order,
    missing, wrong-boundary, and unclosed data are surfaced explicitly.
    """
...
def load_okx_historical_candles(
    ...
    *,
    ...
    while True:
        ...
        page = normalize_okx_history_page(payload, symbol=symbol, timeframe=timeframe)
        if not page:
            break
        ...
        for candle in page:
            ...
            if not candle.is_closed:
                raise UnclosedCandleError(
                    f"OKX returned unconfirmed Candle inside requested closed range: "
                    f"{candle.open_time.isoformat()}"
                )
            if candle.identity in collected:
                raise DuplicateCandleError(
                    f"duplicate Candle across OKX pages: {candle.identity!r}"
                )
        ...
        if next_cursor >= cursor_end_ms:
            raise ProviderResponseError("OKX pagination cursor did not move backwards")
